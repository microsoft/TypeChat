import {Result, TypeChatLanguageModel, createJsonTranslator, TypeChatJsonTranslator, PromptSection} from "typechat";
...
export function createHealthDataTranslator<T extends object>(model: TypeChatLanguageModel, schema: string, type_name: string, additional_agent_instructions: string): TranslatorWithHistory<T> {
    ...
    async function translate(request: string, promptPreamble?: string | PromptSection[]): Promise<Result<T>> {
        const response = await translator.translate(request, promptPreamble);
        if (response.success) {
    ...
    const history_str = JSON.stringify(_chat_history, null, 2);
