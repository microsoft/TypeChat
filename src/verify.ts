...
export function verifyJsonObject(json: object, schema: string, typeName: string) {
    ...
    const host: ts.CompilerHost = {
        ...
        fileName === schemaFilePath ? schemaSourceFile :
        fileName === jsonFilePath ? jsonSourceFile :
        undefined,
        ...
        getNewLine: () => "\r",
        fileExists: fileName => fileName === libFilePath || fileName === schemaFilePath || fileName === jsonFilePath,
        readFile: fileName => "",
    };
    const program = ts.createProgram([libFilePath, schemaFilePath, jsonFilePath], options, host);
    return program.getSemanticDiagnostics().map(d => typeof d.messageText === "string" ? d.messageText : d.messageText.messageText);
}
